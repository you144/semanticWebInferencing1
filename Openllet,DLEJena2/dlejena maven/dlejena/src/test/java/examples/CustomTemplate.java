...
/**
 * This example shows how custom template rules can be added in DLEJena in order
 * to define more complex ABox entailments beyond OWL2 RL.
 *
 * The template rules are rules that dynamically generate the ABox inferencing
 * rules. These rules are executed over the inferred TBox model
 * (PelletInfGraph). Therefore, the ABox rule base of DLEJena is dynamically
 * defined, according to the TBox model.
 *
 * There are two ways of adding template rules in DLEJena. The first is to add
 * directly the custom template rules in the templates.rules file that exists
 * inside dlejena.jar. The second approach is to add them procedurally through
 * the DLEJena API. The present example follows the second approach.
 *
...
public class CustomTemplate {
    ...
    public static void main(String[] args) {
         ...
         */
        IRI ontology = IRI.create(new File("src/test/java/examples/parent.owl").toURI());
    ...
    /**
     * This method defines the template rule for the following generic
     * entailment: [min: (?R owl:minCardinality 1), (?R owl:onProperty ?P), (?X
     * rdf:type ?R), noValue(?X ?P), makeTemp(?T) -> (?X ?P ?T)]
     *
