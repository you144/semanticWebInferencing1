...
/**
 * In this example, we show how DLEJena can handle TBox and ABox updates AFTER
 * BEEN INITIALIZED (DLEJenaReasoner::initialize() method). ABox updates are
 * treated seamlessly, since they can be handled by the entailments that have
 * been generated for the initial TBox KB. However, TBox updates are tricky
 * since the rule based for ABox reasoning should be updated in order to comply
 * with the updated TBox. Currently, DLEJena does not consider any incremental
 * ABox entailment generation procedure, and therefore, the ABox entailments are
 * generated from scratch based on the new TBox. TBox updates are handled by
 * Pellet.
 *
